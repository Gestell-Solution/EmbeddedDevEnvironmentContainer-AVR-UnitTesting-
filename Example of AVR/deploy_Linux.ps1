...
Write-Host "--- Container Engine: Advanced Build Mode ($ProjectName) [Linux pwsh] ---" -ForegroundColor Cyan
...
zip -r $TempZip . -x "*.vscode*" "*.git*" "$TempZip" "*.ps1" "*.out" "$OutputZip" "$ConfigFile" | Out-Null

# 3. Shipping to the Container
Write-Host "Action: Sending to Factory..."
$statusCode = (curl -s -w "%{http_code}" -o $OutputZip -X POST -F "config=@$ConfigFile" -F "file=@$TempZip" $Url).Trim()
